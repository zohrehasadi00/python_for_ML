...
def no_imports(func):
    try:
        ...
        if rujGomsJWH > 0:
            rujGomsJxM(
                "Your function contains {} import".format(rujGomsJWH)
                + f" statement{'s' if rujGomsJWH != 1 else ''} which is not allowed"
            )
    except rujGomsJxt:
        rujGomsJWH = -1
    ...
    def decorator(func):
        try:
            ...
            if rujGomsJWe > rujGomsJWT:
                rujGomsJxM(
                    "Your function uses more loops than allowed"
                    "\nMax allowed: {}\nYour function: {}".format(
                        rujGomsJWT, rujGomsJWe
                    ),
                    file=sys.stderr,
                )
        except rujGomsJxt:
        ...
        def assert_not_too_many_loops():
            assert (
                rujGomsJWT >= rujGomsJWe
            ), "Function uses too many loops: allowed: {},".format(
                rujGomsJWT
            ) + " used: {}".format(
                rujGomsJWe
            )
...
no_loops_allowed = max_allowed_loops(0)
